...
class CFSecurity(Document):
	...
	def _create_financial_periods_from_yahoo(self) -> dict:
		...
		try:
			...
			if self.profit_loss:
				...
				for date_str, pl_data in profit_loss_data.items():
					try:
						...
						# Set new SEC Edgar metadata fields to None (Yahoo doesn't have these)
						period.filing_form = None
						period.accession_number = None
						period.document_fiscal_period = None
						...
						period.save()
						# Commit after each period for transactional safety
						frappe.db.commit()
			...
			# No need for final commit - already committed after each period
				...
				if auto_import_enabled:
					try:
						...
						if self.country == 'United States' and self.sec_cik:
							...
							try:
								from cognitive_folio.utils.sec_edgar_fetcher import fetch_sec_edgar_financials
								result = fetch_sec_edgar_financials(self.name, self.symbol)
								...
								if not result.get('success') or result.get('total_periods', 0) == 0:
									...
									# Log currency mismatches as warning
									frappe.msgprint(
										f"Warning: Currency mismatch in {len(result['currency_mismatches'])} periods. See error log for details.",
										alert=True,
										indicator='orange'
