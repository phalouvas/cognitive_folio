	...
	for holding_data in holdings_data:
		try:
			...
			if existing_holding:
				...
				new_quantity = flt(holding_data.get("quantity") or 0)
				new_price = flt(holding_data.get("average_purchase_price") or 0)
				new_cost = new_quantity * new_price
				new_total_quantity = existing_doc.quantity + new_quantity
				...
				new_holding = frappe.get_doc({
					...
					"portfolio": holding_data.get("portfolio"),
					"security": holding_data.get("security"),
					"quantity": flt(holding_data.get("quantity") or 0),
					"average_purchase_price": flt(holding_data.get("average_purchase_price") or 0)
				})
